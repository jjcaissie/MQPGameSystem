...
global askedQuestions, characterToGuess, numQuestions, characters, gameState
numQuestions = 5
numGuesses = 0
...
def StartGame(charNotToPick = ''):
    global characterToGuess, characterAttributes, askedQuestions, numGuesses, gameState
    characterToGuess, characterAttributes = GetCharacter(charNotToPick)
    askedQuestions = -1
    numGuesses = 0
    gameState = 1
    answer = "I'm thinking of a character. Guess who!"
    return render_template('headbands2.html', answer=answer)

#Handles user's submitted questions
def AskQuestion(userQuestion):
    global askedQuestions, numQuestions, gameState
    askedQuestions += 1    
    ...
    if(askedQuestions >= numQuestions):                  #check if game should end based on number of questions
        ...
        return render_template("headbands2.html", answer=answer, firstGuess=True)
...
def GuessCharacter(userGuess):
    ...
    if(userGuess.lower() != characterToGuess):
        if(numGuesses == 0):
            ...
            numGuesses+=1
            return render_template("headbands2.html", answer=answer, secondGuess=True, characters=characters)
        answer = "That's not it! I was thinking of " + characterToGuess +"! Would you like to continue playing?"
        return render_template("headbands2.html", answer=answer, promptReplay=True)     #User did not guess correctly on second try
    return render_template("headbands2.html", answer=answer, promptReplay=True)         #User guessed correctly
